...
def get_bin_logs():
    ...
    try:
        ...
        mav_conn.mav.log_request_list_send(
            mav_conn.target_system, mav_conn.target_component, 0, 0xFFFF
        )
        ...
        while retries > 0:
            ...
            if message:
                logger.debug(
                    "Received log entry: ID=%s, Size=%s, Time UTC=%s",
                    message.id,
                    message.size,
                    message.time_utc,
                )
        ...
        mav_conn.mav.log_request_end_send(mav_conn.target_system, mav_conn.target_component)
        mav_conn.close()
...
def download_bin_log():
    ...
    try:
        ...
        # Pad the log_id with zeros to 8 digits and append .BIN extension.
        formatted_log_id = f"{log_id_int:08d}.BIN"
    ...
    logger.info(
        "Heartbeat from MAVLink system (ID %d component %d)",
    ...
    master.mav.log_request_data_send(
        master.target_system, master.target_component, log_id_int, 0, 0xFFFFFFFF
    )
