    ...
    "targets": [
        {
            ...
            "conditions": [
                [
                    'OS=="linux"',
                    {
                        "cflags": ["<!@(pkg-config --cflags gazebo jansson protobuf)"],
                        "cflags_cc": ["-fno-rtti", "-fno-exceptions", "-std=c++17"],
                        "ldflags": [
                            "<!@(pkg-config --libs-only-L --libs-only-other gazebo jansson protobuf)"
                        ],
                        "libraries": ["<!@(pkg-config --libs-only-l gazebo jansson protobuf)"],
                    },
                ],
                [
                    'OS=="mac"',
                    {
                        ...
                        "xcode_settings": {
                            ...
                            "OTHER_CFLAGS": ["<!@(pkg-config --cflags gazebo jansson protobuf)"],
